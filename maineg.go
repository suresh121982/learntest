...
func main() {
	...
	//addresses := []string{"192.168.1.100:8000", "192.168.1.103:8000"}
	addresses := []string{"192.168.1.103:8000", "192.168.1.104:8000"}
	...
	for _, addr := range addresses {
		...
		go func(address string) {
			defer wg.Done()
			logging.Log("INFO", fmt.Sprintf("Server running on %s", address))
			fmt.Printf("Server running on %s\n", address)
			if err := http.ListenAndServe(address, router); err != nil {
				log.Fatalf("Server error on %s: %v", address, err)
			}
		}(addr)
	}
